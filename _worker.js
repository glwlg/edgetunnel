...
function 获取Pages静态页面(env = {}) {
	...
	try {
		let normalized = String(自定义Pages静态页面).trim().replace(/\/+$/, '');
...
async function 获取Pages静态页面响应(Pages静态页面, 页面路径) {
	const 问号索引 = 页面路径.indexOf('?');
	const 路径部分 = 问号索引 === -1 ? 页面路径 : 页面路径.slice(0, 问号索引);
	const 查询部分 = 问号索引 === -1 ? '' : 页面路径.slice(问号索引);
	const 规范路径 = 路径部分.startsWith('/') ? 路径部分 : '/' + 路径部分;
	const 响应 = await fetch(Pages静态页面 + 规范路径 + 查询部分);
	const 文件名 = 规范路径.split('/').pop() || '';
	const 最终响应 = 响应.ok || 文件名.includes('.')
		? 响应
		: await fetch(Pages静态页面 + 规范路径.replace(/\/$/, '') + '/index.html' + 查询部分);
	const headers = new Headers(最终响应.headers);
	if (!headers.get('Content-Type') || headers.get('Content-Type').includes('text/plain')) headers.set('Content-Type', 'text/html; charset=UTF-8');
	return new Response(最终响应.body, { status: 最终响应.status, statusText: 最终响应.statusText, headers });
}
///////////////////////////////////////////////////////全局常量和工具函数///////////////////////////////////////////////
...
export default {
	async fetch(request, env, ctx) {
		const Pages静态页面 = 获取Pages静态页面(env);
		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			if (!管理员密码) return 获取Pages静态页面响应(Pages静态页面, '/noADMIN').then(r => { const headers = new Headers(r.headers); headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); headers.set('Pragma', 'no-cache'); headers.set('Expires', '0'); return new Response(r.body, { status: 404, statusText: r.statusText, headers }) });
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					return 获取Pages静态页面响应(Pages静态页面, '/login');
					...
					if (访问路径 === 'admin/init') {// 重置配置为默认值
						...
						if (访问路径 === 'admin/config.json') { // 保存config.json配置
						...
						} else if (访问路径 === 'admin/bestip.json') { // 保存场景优选结果
							try {
								...
								const saved = await 保存场景优选记录(env, profile, records);
								ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Save_Profile_BestIP', config_JSON));
								return new Response(JSON.stringify({ success: true, profile, label: 获取场景档案(profile).label, count: saved.length, records: saved }, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							} catch (error) {
							...
							try {
								...
								const profile = 规范化优选场景(url.searchParams.get('profile'));
								if (profile) {
									const records = customIPs.split(/\r?\n/).map(line => 标准化优选记录(line, profile)).filter(Boolean);
									const saved = await 保存场景优选记录(env, profile, records);
									ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Save_Profile_Custom_IPs', config_JSON));
									return new Response(JSON.stringify({ success: true, message: '场景优选IP已保存', profile, count: saved.length }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
								}
					...
					} else if (访问路径 === 'admin/bestip.json') {// 场景优选结果
						const profile = 规范化优选场景(url.searchParams.get('profile')) || 识别优选场景(request, url);
						const records = await 读取场景优选记录(env, profile);
						const fallback = await 获取订阅优选列表(env, request, config_JSON, url);
						return new Response(JSON.stringify({
							success: true,
							profile,
							label: 获取场景档案(profile).label,
							records,
							lines: records.map(record => 优选记录转行(record, profile)),
							fallbackSource: fallback.source,
							fallbackLines: fallback.lines,
						}, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
					} else if (访问路径 === 'admin/bestip-candidates.json') {// 生成/读取 OpenClash 场景候选池
						const profile = 规范化优选场景(url.searchParams.get('profile')) || 识别优选场景(request, url);
						const records = await 生成并保存场景候选记录(env, request, config_JSON, url, profile);
						const batch = url.searchParams.get('batch') || String(Date.now());
						return new Response(JSON.stringify({
							success: true,
							profile,
							label: 获取场景档案(profile).label,
						...
						if (profile) {
							const records = await 读取场景优选记录(env, profile);
							return new Response(records.map(record => 优选记录转行(record, profile)).join('\n'), { status: 200, headers: { 'Content-Type': 'text/plain;charset=utf-8', 'X-GLWLG-Profile': profile, 'X-GLWLG-Profile-Label': encodeURIComponent(获取场景档案(profile).label) } });
						}
					...
					return 获取Pages静态页面响应(Pages静态页面, '/admin' + url.search);
					...
					const 兼容订阅TOKEN列表 = [订阅TOKEN];
					if (env.HOST) {
						const legacyHosts = (await 整理成数组(env.HOST)).map(h => h.toLowerCase().replace(/^https?:\/\//, '').split('/')[0].split(':')[0]).filter(Boolean);
						兼容订阅TOKEN列表.push(...await Promise.all(legacyHosts.map(legacyHost => MD5MD5(legacyHost + userID))));
					}
					const 用户客户端请求订阅 = 兼容订阅TOKEN列表.includes(请求TOKEN);
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						const 订阅场景 = 识别优选场景(request, url);
						const responseHeaders = {
							"content-type": "text/plain; charset=utf-8",
							"Profile-Update-Interval": config_JSON.优选订阅生成.SUBUpdateTime,
							"Profile-web-page-url": url.protocol + '//' + url.host + '/admin',
							"Cache-Control": "no-store",
							"X-GLWLG-Profile": 订阅场景,
							"X-GLWLG-Profile-Label": encodeURIComponent(获取场景档案(订阅场景).label),
						};
						...
						if (订阅类型 === 'glwlg-mihomo' || url.searchParams.has('mihomo-test') || url.searchParams.has('glwlg-mihomo')) {
							const 场景列表 = url.searchParams.has('candidate')
								? await 获取场景候选订阅列表(env, request, config_JSON, url, 订阅场景)
								: await 获取订阅优选列表(env, request, config_JSON, url);
							订阅内容 = 生成Mihomo测试YAML(场景列表.records, config_JSON, 场景列表.profile, userID, url.searchParams.get('batch') || '');
							responseHeaders["content-type"] = 'application/x-yaml; charset=utf-8';
							responseHeaders["X-GLWLG-Profile-Source"] = 场景列表.source;
							return new Response(订阅内容, { status: 200, headers: responseHeaders });
						}
						if (订阅类型 === 'mixed') {
							...
							if (!url.searchParams.has('sub') && config_JSON.优选订阅生成.local) { // 本地生成订阅
								const 场景列表 = await 获取订阅优选列表(env, request, config_JSON, url);
								responseHeaders["X-GLWLG-Profile-Source"] = 场景列表.source;
								const 完整优选列表 = 场景列表.lines;
							...
							const 订阅转换URL = `${config_JSON.订阅转换配置.SUBAPI}/sub?target=${订阅类型}&url=${encodeURIComponent(url.protocol + '//' + url.host + '/sub?target=mixed&token=' + 今日订阅转换后端专属TOKEN + '&asOrg=' + 获取场景档案(订阅场景).asOrg + '&profile=' + 订阅场景 + (url.searchParams.has('sub') && url.searchParams.get('sub') != '' ? `&sub=${url.searchParams.get('sub')}` : ''))}&config=${encodeURIComponent(config_JSON.订阅转换配置.SUBCONFIG)}&emoji=${config_JSON.订阅转换配置.SUBEMOJI}&scv=${config_JSON.跳过证书验证}`;
							try {
			...
			} else if (!envUUID) return 获取Pages静态页面响应(Pages静态页面, '/noKV').then(r => { const headers = new Headers(r.headers); headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); headers.set('Pragma', 'no-cache'); headers.set('Expires', '0'); return new Response(r.body, { status: 404, statusText: r.statusText, headers }) });
		}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	config_JSON.HOSTS = [hostname.toLowerCase()];
...
function 获取场景档案(profile) {
	return 场景优选档案[profile] || 场景优选档案['cf-fallback'];
}

function 规范化优选场景(profile) {
	const value = String(profile || '').trim().toLowerCase();
	if (value && value !== 'auto' && 场景优选档案[value]) return value;
	return null;
}

function 运营商转优选场景(asOrg) {
	const value = String(asOrg || '').trim().toLowerCase();
	if (value === 'cmcc') return 'cmcc-js-home';
	if (value === 'cu') return 'cu-mobile';
	return 'cf-fallback';
}

function 识别优选场景(request, url = null) {
	const searchParams = url?.searchParams || new URL(request.url).searchParams;
	const 显式场景 = 规范化优选场景(searchParams.get('profile'));
	if (显式场景) return 显式场景;
	const 显式运营商 = String(searchParams.get('asOrg') || '').trim().toLowerCase();
	if (['cmcc', 'cu', 'ct', 'cf'].includes(显式运营商)) return 运营商转优选场景(显式运营商);
	return 运营商转优选场景(识别运营商(request));
}

function 解析优选地址行(line) {
	const 文本 = String(line || '').trim();
	if (!文本) return null;
	const 备注索引 = 文本.indexOf('#');
	const 地址端口部分 = (备注索引 === -1 ? 文本 : 文本.slice(0, 备注索引)).trim();
	const 备注 = 备注索引 === -1 ? '' : 文本.slice(备注索引 + 1).trim();
	if (!地址端口部分 || 地址端口部分.includes('://')) return null;
	let address = '', port = 443;
	const ipv6 = 地址端口部分.match(/^\[([0-9a-fA-F:.]+)](?::(\d{1,5}))?$/);
	if (ipv6) {
		address = ipv6[1];
		port = ipv6[2] ? Number(ipv6[2]) : 443;
	} else {
		const colonIndex = 地址端口部分.lastIndexOf(':');
		if (colonIndex > -1 && /^\d{1,5}$/.test(地址端口部分.slice(colonIndex + 1))) {
			address = 地址端口部分.slice(0, colonIndex);
			port = Number(地址端口部分.slice(colonIndex + 1));
		} else {
			address = 地址端口部分;
		}
	}
	if (!address || !Number.isInteger(port) || port < 1 || port > 65535) return null;
	return { address: address.replace(/^\[|]$/g, ''), port, remark: 备注 };
}

function 格式化优选地址端口(address, port = 443) {
	const clean = String(address || '').replace(/^\[|]$/g, '');
	return clean.includes(':') ? `[${clean}]:${port}` : `${clean}:${port}`;
}
...
function 获取记录地区标签(record, scene) {
	const country = 获取记录国家代码(record, scene);
	const flag = 获取记录旗标(record, scene);
	const countryName = 获取国家显示名称(country);
	const colo = String(record.colo || '').trim().toUpperCase();
	const city = String(record.city || '').trim();
	const region = String(record.regionCode || record.region || '').trim();
	const parts = [];
	if (flag) parts.push(flag);
	if (countryName) parts.push(countryName);
	if (colo) parts.push(colo);
	else if (city) parts.push(city);
	else if (region && region.toUpperCase() !== country) parts.push(region);
	return parts.join(' ').trim();
}

function 去除备注地区前缀(remark, record, scene) {
	let text = 去除备注旗标(remark);
	const country = 获取记录国家代码(record, scene);
	const countryNames = [获取国家显示名称(country), country].filter(Boolean);
	for (const name of countryNames) {
		if (text.startsWith(name)) text = text.slice(name.length).trim();
	}
	const colo = String(record.colo || '').trim().toUpperCase();
	if (colo && text.toUpperCase().startsWith(colo)) text = text.slice(colo.length).trim();
	return text;
}

function 构建优选记录备注(record, profile = 'cf-fallback') {
	const scene = 获取场景档案(profile);
	const location = 获取记录地区标签(record, scene);
	const rawRemark = 去除备注地区前缀(record.remark || scene.label, record, scene);
	const sceneRemark = rawRemark.includes(scene.label) ? rawRemark : `${scene.label} ${rawRemark}`.trim();
	return `${location} ${sceneRemark}`.trim();
}
...
function 标准化优选记录(record, profile = 'cf-fallback') {
	...
	const 拼接地址 = record.line || record.raw || record.value || (
		record.address ? `${record.address}${record.port ? ':' + record.port : ''}${record.remark ? '#' + record.remark : ''}` : ''
	);
...
function 排序优选记录(records) {
	const 去重 = new Map();
	for (const record of records) {
		if (!record) continue;
		const key = `${record.address}:${record.port}`;
		const old = 去重.get(key);
		if (!old || 比较优选记录(record, old) < 0) 去重.set(key, record);
	}
	return [...去重.values()].sort(比较优选记录);
}
...
function 比较优选记录(a, b) {
	...
	const aInfo = 记录地区完整度(a), bInfo = 记录地区完整度(b);
	if (aInfo !== bInfo) return bInfo - aInfo;
	return `${a.address}:${a.port}`.localeCompare(`${b.address}:${b.port}`);
}

function 记录地区完整度(record) {
	return (获取记录国家代码(record, 获取场景档案('cf-fallback')) ? 1 : 0)
		+ (record.colo ? 1 : 0)
		+ (record.continent ? 1 : 0)
		+ (record.city ? 1 : 0);
}

function 获取记录地区分组(record) {
	const country = 获取记录国家代码(record, 获取场景档案('cf-fallback'));
	if (中国地区国家代码.has(country)) return 'china';
	if (country === 'US') return 'us';
	if (country === 'JP') return 'jp';
	if (欧洲国家代码.has(country) || String(record.continent || '').trim().toUpperCase() === 'EU') return 'europe';
	return 'other';
}

function 按地区配额筛选优选记录(records, limit = 场景优选保存数量) {
	const selected = [], selectedKeys = new Set(), groupCounts = { china: 0, us: 0, jp: 0, europe: 0, other: 0 };
	const addRecord = (record) => {
		if (!record || selected.length >= limit) return false;
		const key = `${record.address}:${record.port}`;
		if (selectedKeys.has(key)) return false;
		const group = 获取记录地区分组(record);
		if (group === 'china' && groupCounts.china >= 场景优选中国地区数量上限) return false;
		selected.push(record);
		selectedKeys.add(key);
		groupCounts[group] = (groupCounts[group] || 0) + 1;
		return true;
	};
	const pickGroup = (group, count) => {
		let added = 0;
		for (const record of records) {
			if (added >= count || selected.length >= limit) break;
			if (获取记录地区分组(record) !== group) continue;
			if (addRecord(record)) added++;
		}
		return added;
	};
	pickGroup('china', 场景优选中国地区数量上限);
	pickGroup('us', 场景优选美国数量);
	pickGroup('jp', 场景优选日本数量);
	const europeAdded = pickGroup('europe', 场景优选欧洲数量);
	if (europeAdded < 场景优选欧洲数量) pickGroup('jp', 场景优选欧洲数量 - europeAdded);
	for (const record of records) {
...
function 优选记录转行(record, profile = 'cf-fallback') {
	const scene = 获取场景档案(profile);
	const metrics = [];
	if (Number.isFinite(record.clashDelayMs)) metrics.push(`Clash ${Math.round(record.clashDelayMs)}ms`);
...
async function 读取场景优选记录(env, profile) {
	const scene = 获取场景档案(profile);
	try {
		const raw = await env.KV.get(scene.kvKey);
		if (!raw) return [];
		const data = JSON.parse(raw);
		const records = Array.isArray(data) ? data : (Array.isArray(data.records) ? data.records : []);
		return 按地区配额筛选优选记录(排序优选记录(records.map(item => 标准化优选记录(item, profile)).filter(Boolean)), 场景优选保存数量);
	} catch (error) {
...
async function 保存场景优选记录(env, profile, inputRecords) {
	const scene = 获取场景档案(profile);
	const records = 按地区配额筛选优选记录(排序优选记录((Array.isArray(inputRecords) ? inputRecords : []).map(item => 标准化优选记录(item, profile)).filter(Boolean)), 场景优选保存数量);
	await env.KV.put(scene.kvKey, JSON.stringify({ profile, label: scene.label, updatedAt: new Date().toISOString(), records }, null, 2));
	return records;
}

function 获取候选KVKey(profile) {
	return `bestip:candidate:${规范化优选场景(profile) || 'cf-fallback'}`;
}

function 解析候选端口(url) {
	const raw = String(url.searchParams.get('ports') || url.searchParams.get('port') || '443,2053,2083,2087,2096,8443');
	const ports = raw.split(/[,，\s]+/)
		.map(port => Number(port))
		.filter(port => Number.isInteger(port) && port >= 1 && port <= 65535);
	const uniquePorts = [...new Set(ports)];
	return uniquePorts.length ? uniquePorts : [443, 2053, 2083, 2087, 2096, 8443];
}

function 解析候选数量(url) {
	const count = Number(url.searchParams.get('count') || 96);
	if (!Number.isFinite(count)) return 96;
...
function 保序标准化优选记录(records, profile, limit = 240) {
	...
	for (const item of Array.isArray(records) ? records : []) {
		const record = 标准化优选记录(item, profile);
		if (!record) continue;
		const key = `${record.address}:${record.port}`;
...
async function 读取场景候选记录(env, profile) {
	try {
		const raw = await env.KV.get(获取候选KVKey(profile));
		if (!raw) return [];
		const data = JSON.parse(raw);
		const records = Array.isArray(data) ? data : (Array.isArray(data.records) ? data.records : []);
		return 保序标准化优选记录(records, profile, 240);
	} catch (error) {
		console.error(`[场景候选] 读取失败 ${profile}:`, error);
		return [];
	}
}

async function 保存场景候选记录(env, profile, inputRecords) {
	const scene = 获取场景档案(profile);
	const records = 保序标准化优选记录(inputRecords, profile, 240);
	await env.KV.put(获取候选KVKey(profile), JSON.stringify({ profile, label: scene.label, updatedAt: new Date().toISOString(), records }, null, 2));
	return records;
}
...
async function 获取场景候选订阅列表(env, request, config_JSON, url, profile) {
	let records = await 读取场景候选记录(env, profile);
	if (!records.length) records = await 生成并保存场景候选记录(env, request, config_JSON, url, profile);
	return { profile, records, lines: records.map(record => 优选记录转行(record, profile)), source: 'candidate' };
}

async function 获取订阅优选列表(env, request, config_JSON, url) {
	const profile = 识别优选场景(request, url);
	const records = await 读取场景优选记录(env, profile);
	if (records.length > 0) {
		return { profile, records, lines: records.map(record => 优选记录转行(record, profile)), source: 'profile' };
	}
...
function 生成Mihomo测试YAML(records, config_JSON, profile, userID, batch = '', options = {}) {
	...
	const batchLabel = String(batch || '').replace(/[^\w.-]/g, '').slice(-8);
	const namePrefix = `GLWLG-${scene.label}${batchLabel ? '-' + batchLabel : ''}`;
	...
	const proxies = records.map((record, index) => {
		...
		const base = [
			`  - name: ${yamlQuote(name)}`,
			`    type: ${config_JSON.协议类型 === 'trojan' ? 'trojan' : 'vless'}`,
			`    server: ${yamlQuote(server)}`,
			`    port: ${Number(record.port) || 443}`,
			config_JSON.协议类型 === 'trojan' ? `    password: ${yamlQuote(userID)}` : `    uuid: ${yamlQuote(userID)}`,
			`    udp: true`,
			`    tls: true`,
			`    servername: ${yamlQuote(sniHost)}`,
			`    skip-cert-verify: ${Boolean(config_JSON.跳过证书验证)}`,
			`    client-fingerprint: ${yamlQuote(config_JSON.Fingerprint || 'chrome')}`,
		];
		if (传输协议.startsWith('grpc')) {
			base.push(`    network: grpc`);
			base.push(`    grpc-opts:`);
			base.push(`      grpc-service-name: ${yamlQuote(path.replace(/^\//, '') || 'grpc')}`);
			if (config_JSON.gRPCUserAgent) base.push(`      grpc-user-agent: ${yamlQuote(config_JSON.gRPCUserAgent)}`);
		} else {
			base.push(`    network: ws`);
			base.push(`    ws-opts:`);
			base.push(`      path: ${yamlQuote(path)}`);
			base.push(`      headers:`);
			base.push(`        Host: ${yamlQuote(sniHost)}`);
		}
...
async function 生成随机IP(request, count = 16, 指定端口 = -1, 指定运营商 = null, 候选端口列表 = null) {
	const url = new URL(request.url);
	const 查询参数运营商 = String(url.searchParams.get('asOrg') || '').toLowerCase();
	const 手动运营商 = String(指定运营商 || '').toLowerCase();
	const 运营商文件标识 = ['ct', 'cu', 'cmcc', 'cf'].includes(手动运营商)
		? 手动运营商
		: ['ct', 'cu', 'cmcc', 'cf'].includes(查询参数运营商) ? 查询参数运营商 : 识别运营商(request);
	const 运营商名称映射 = {
		cmcc: 'CF移动优选',
		cu: 'CF联通优选',
		ct: 'CF电信优选',
	...
	const 可用端口 = Array.isArray(候选端口列表) && 候选端口列表.length ? 候选端口列表 : cfport;
	let cidrList = [];
	...
	const randomIPs = Array.from({ length: count }, (_, index) => {
		...
		const 目标端口 = 指定端口 === -1
			? (Array.isArray(候选端口列表) && 候选端口列表.length ? 可用端口[index % 可用端口.length] : cfport[Math.floor(Math.random() * cfport.length)])
			: 指定端口;
