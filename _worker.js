				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'glwlg-mihomo' || url.searchParams.has('mihomo-test') || url.searchParams.has('glwlg-mihomo')) {
							...
							订阅内容 = 生成Mihomo测试YAML(场景列表.records, config_JSON, 场景列表.profile, userID);
...
function 生成Mihomo测试YAML(records, config_JSON, profile, userID) {
	const scene = 获取场景档案(profile);
	...
	const proxies = records.map((record, index) => {
		const name = `GLWLG-${scene.label}-${String(index + 1).padStart(2, '0')}-${record.colo || record.address}`;
...
async function 生成随机IP(request, count = 16, 指定端口 = -1, 指定运营商 = null) {
